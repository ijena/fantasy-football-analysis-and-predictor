...
INPUT_DIR = BASE_DIR / r"Pro Football Reference Fantasy Rank 2015-2024"
...
def flatten_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(df.columns, pd.MultiIndex):
        ...
        # single row header; just strip 'Unnamed'
        df.columns = [
...
def add_helpers(df: pd.DataFrame, year: int) -> pd.DataFrame:
    """
    Add Player_fixed, player_clean, team, POS_group, year, PPR_rank_year (overall rank by PPR within year).
    ...
    if pos_col is not None:
        # keep original POS column name; also make a normalized version POS (upper)
        if "POS" not in df.columns:
            df["POS"] = df[pos_col]
        df["POS_group"] = df["POS"].astype(str).str.extract(r"([A-Za-z]+)", expand=False).str.upper()
    else:
        df["POS_group"] = pd.NA
    ...
    # PPR yearly rank (overall rank within the year; 1 = best)
    if ppr_col is None:
        # try to guess a PPR column if flattened headers changed names
    ...
    if ppr_col is not None:
        ...
        # rank within this dataframe (all rows are same year here)
...
def load_one_file(path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        df = flatten_columns(df)
    # Drop completely empty columns (common with wide exports)
